  ...
  <div class="mod-group">
    <el-form :inline="true" :model="searchForm" @keyup.enter.native="getDataList(1)">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('ad:group:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('ad:group:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="showMachineList(scope.row.id)">{{scope.row.groupName}}</el-button>
        </template>
      </el-table-column>
      <el-table-column
        prop="nickname"
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
<!--          <el-button v-if="isAuth('ad:group:info')" type="text" size="small" @click="showDetails(scope.row.id)">查看</el-button>-->
<!--          <el-button v-if="isAuth('ad:group:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>-->
          <el-button v-if="isAuth('ad:group:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">追加</el-button>
          <el-button v-if="isAuth('ad:group:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <el-dialog
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="操作">
          <template slot-scope="scope">
            <el-button  type="text" size="small" @click="machineGroupDeleteHandle(scope.row.machineGroupId)">删除分组</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[删除]操作?`, '提示', {
          ...
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              this.getMachineList(this.parentId)
            }
          })
        }).catch(() => {
        })
      ...
      getMachineList (id) {
        this.$http({
          url: '/ad/machinegroup/getMachineByGroupId',
          method: 'get',
          params: {
